...
public class ReqnrollCompletionHandler : CompletionHandlerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var lineBeforeCursor = currentLine.Substring(0, Math.Min((int)request.Position.Character, currentLine.Length)).TrimStart();

        // If document is empty or only whitespace, offer Feature keyword
        if (string.IsNullOrWhiteSpace(documentContent))
        {
            return Task.FromResult(new CompletionList(new[] {
                CreateBlockKeywordCompletion("Feature", "Defines a feature containing scenarios")
            }, isIncomplete: false));
        }

        var isInsideBlock = IsInsideScenarioOrBackground(lines, request.Position);

        // If no text typed yet, offer appropriate keywords
        if (string.IsNullOrWhiteSpace(lineBeforeCursor))
        {
            if (isInsideBlock)
            {
                // Inside scenario: offer step keywords
                return Task.FromResult(new CompletionList(new[] {
                    CreateStepKeywordCompletion("Given", "Precondition or context setup"),
                    CreateStepKeywordCompletion("When", "Action or event"),
                    CreateStepKeywordCompletion("Then", "Expected outcome or result"),
                    CreateStepKeywordCompletion("And", "Additional step of same type as previous"),
                    CreateStepKeywordCompletion("But", "Negative additional step of same type as previous")
                }, isIncomplete: false));
            }
            else
            {
                // Outside scenario: offer block keywords
                return Task.FromResult(new CompletionList(new[] {
                    CreateBlockKeywordCompletion("Background", "Steps run before each scenario"),
                    CreateBlockKeywordCompletion("Scenario", "A test scenario"),
                    CreateBlockKeywordCompletion("Scenario Outline", "A parameterized scenario template"),
                    CreateBlockKeywordCompletion("Examples", "Data table for Scenario Outline")
                }, isIncomplete: false));
            }
        }

        // Must be inside a block to get step completions
        if (!isInsideBlock)
        {
            return Task.FromResult(new CompletionList(isIncomplete: false));
        }
        ...
        var requestedStepType = DetermineStepType(lineBeforeCursor, lines, request.Position);
        ...
        var hasKeyword = HasStepKeyword(lineBeforeCursor);
    ...
    /// <summary>
    /// Creates a block keyword completion item (with colon).
    /// </summary>
    private CompletionItem CreateBlockKeywordCompletion(string keyword, string description)
    {
    ...
    /// <summary>
    /// Creates a step keyword completion item (with space).
    /// </summary>
    private CompletionItem CreateStepKeywordCompletion(string keyword, string description)
    {
        return new CompletionItem
        {
            Label = keyword,
